...
(async () => {
  ...
  await insb('vlan', r.map((x) => ({ legacyId: x.idv, branchId: bid(x.sede), sedeLegacy: x.sede, vlan: x.vlan, olt: x.olt, tray: x.bandeja, oltPort: x.puertolt, detail: x.det_vlan || '' })));
  ...
  await insb('nap', r.map((x) => ({ legacyId: x.idn, branchId: bid(x.sede), sedeLegacy: x.sede, vlanId: vlan.get(x.idvlan) || null, vlanLegacy: x.idvlan, name: x.nap, portCount: x.puertos, address: x.dir_nap, gpsLat: x.coor1, gpsLng: x.coor2 })));
  ...
  await insb('equipmentWarehouse', r.map((x) => ({ legacyId: x.id, name: x.almacen, description: x.descripcion })));
