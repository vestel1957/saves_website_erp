...
import { cobranzasService, facturasService, pagosFijosService, treasuryService } from '../core/contenedor';
...
const treasury = new TreasuryController(treasuryService, cobranzasService, pagosFijosService, facturasService);
...
treasuryRouter.get(
  ...
  autenticar,
  exigirArea('contabilidad', 'administracion', 'caja'),
  manejar((req) => treasury.cashAccounts(usuarioDe(req), req.query.from as string, req.query.to as string)),
);
...
treasuryRouter.get(
  ...
  autenticar,
  exigirArea('contabilidad', 'administracion', 'caja'),
  manejar((req) => treasury.cashCloseWompi(req.query.cashAccountId as string, req.query.date as string, usuarioDe(req))),
);
...
treasuryRouter.get(
  ...
  autenticar,
  exigirArea('contabilidad', 'administracion', 'caja'),
  manejar((req) => treasury.stats(req.query.from as string, req.query.to as string, req.query.all as string, req.query.sede as string, usuarioDe(req))),
);
