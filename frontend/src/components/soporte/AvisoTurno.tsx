...
import type { MiTurno } from "@/components/soporte/VisitaEnTurno";

/**
 * Aviso en "Mis órdenes de trabajo": esta lista es tu historial, no tu menú.
 *
 * Desde el turno obligatorio (2026-08-04), un técnico con visita en turno solo puede
 * abrir esa. La lista sigue existiendo —es donde consulta lo que ya hizo, y quitársela
 * lo dejaría sin memoria de su trabajo—, pero sin este aviso el candado se vive como
 * una avería: toca una fila, le sale un error rojo y no entiende por qué.
 *
...
export function AvisoTurno() {
  ...
  const [t, setT] = useState<MiTurno | null>(null);

  useEffect(() => {
    void authFetch("/support/mi-turno")
      .then((r) => (r.ok ? r.json() : null))
      .then(setT)
      .catch(() => undefined);
  }, [authFetch]);

  if (!t?.visita) return null;
    ...
    <div className="flex flex-col gap-2 rounded-xl border border-brand/40 bg-brand-soft/30 px-3.5 py-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-start gap-2">
        ...
        <p className="text-[12.5px] text-text-secondary">
          Tienes una visita en turno: <b className="text-text-primary">{t.visita.type}</b>
          {t.visita.cliente ? ` · ${t.visita.cliente}` : ""}. Las visitas se atienden en el orden que
