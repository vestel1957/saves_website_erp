        ...
        <div className="inline-flex rounded-lg border border-border-default bg-surface p-0.5">
          {(["compra", "servicio"] as const).map((k) => (
            <button
              ...
              type="button"
              onClick={() => setKind(k)}
              className={`rounded-md px-3.5 py-1.5 text-[13px] font-semibold capitalize transition-colors ${kind === k ? "bg-brand text-on-brand" : "text-text-secondary hover:bg-surface-2"}`}
            >
              {k}
            </button>
          ))}
