/**
 * Alta de la pantalla /afiliados (códigos de afiliado, 2026-09-23). Idempotente.
 *
 * Siembra el catálogo de permisos y concede `screen.afiliados` a los roles de las
 * áreas que declara SCREENS (administración y gerencia). Sin esto la entrada del menú
 * sólo la ve el superusuario (ver la nota «pantallas sin llave»).
 *
 * Correr:  npx ts-node --transpile-only prisma/migrate-pantalla-afiliados-2026-09.ts
 */
import { PrismaClient } from '@prisma/client';
import { ALL_PERMISSIONS, SCREENS, screenKey } from '../src/auth/permissions.catalog';

const prisma = new PrismaClient();
const HREF = '/afiliados';
