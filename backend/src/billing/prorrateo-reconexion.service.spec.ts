...
function servicioDePrueba(opts: {
  ajuste?: string | null;
  servicios?: Servicio[];
  factura?: { id: string; tid: number; status: string; total: number; paidAmount: number; items: Item[] } | null;
  ...
  const prisma: any = {
    ...
    subInvoice: {
      findFirst: async () =>
        opts.factura ? { ...opts.factura, subtotal: opts.factura.total, tax: 0, electronicInvoices: [] } : null,
    },
    ...
    // El respaldo "plan de su última factura" va por SQL crudo; aquí no hay ninguno.
    $queryRaw: async () => [],
  };
