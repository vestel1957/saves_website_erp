...
import { OltController, AdoptarDto, CatvSetDto, CatvStateDto, ConfigurarVlanEquiposDto, LinkDto, OltUpsertDto, OnuActionDto, OnuDescDto, OnusQueryDto, PlanOltProfileDto, PlanOltProfileLoteDto, ProvisionDto, SlotDto } from './olt.controller';
import { oltPlanProfileService, oltService, vlanEquiposService } from '../core/contenedor';
...
const olt = new OltController(oltService, oltPlanProfileService, vlanEquiposService);
...
oltRouter.get(
  ...
  exigirArea('tecnicos', 'administracion'),
  moduloRed,
  manejar((req) => olt.mapaVlans(req.query.branchId as string, req.query.refresh as string)),
);
...
oltRouter.get(
  ...
  exigirArea('tecnicos', 'administracion'),
  moduloRed,
  manejar((req) => olt.saludVlans(req.query.branchId as string, req.query.refresh as string)),
);
...
oltRouter.get(
  ...
  exigirArea('tecnicos', 'administracion'),
  moduloRed,
  manejar((req) => olt.sugerirVlan(req.query.oltId as string, req.query.slot as string, req.query.port as string)),
);

oltRouter.post(
  '/vlans/:id/configurar-equipos',
  autenticar,
  exigirArea('tecnicos', 'administracion'),
  moduloRed,
  manejar((req) => olt.configurarVlanEquipos(req.params.id, validar(ConfigurarVlanEquiposDto, req.body), usuarioDe(req))),
);
